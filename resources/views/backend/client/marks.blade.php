                    ...
                    <div class="row">
                        <div class="col-lg-8">
                            <div class="navigation_title">
                                ...
                                <b class="ml-2"><i class="far fa-clock"></i> Invited On :
                                </b>{{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $candidateInfo->invited_on)->format('d M, Y h:i A') }}
                        ...
                        <div class="col-lg-4 text-right">
                            <div class="navigation_item pt-3">
                                <ul>
                                    <li class="active">
                                        <button type="submit" style="cursor: pointer"><i class="fas fa-check"></i> Submit
                                            Grades</button>
                                    </li>
                                </ul>
                            </div>
                                            ...
                                            @foreach ($results as $index => $result)
                                                <tr>
                                                    <th scope="row">
                                                        {{ $sl }}
                                                        @csrf
                                                        <input type="hidden" name="candidate_slug"
                                                            value="{{ $candidateInfo->slug }}">
                                                        <input type="hidden" name="question_id[]"
                                                            value="{{ $result->question_id }}">
                                                    </th>
                                                    <td scope="row">
                                                        @if (($result->question_type == 1 || $result->question_type == 3) && $result->question_created_by != Auth::user()->id)
                                                            <i style="font-weight: 600;">Protecting Test Integrity :
                                                                Questions and Answers Hidden from Employers</i>
                                                        @else
                                                            {!! $result->question !!}
                                                        @endif

                                                        @php
                                                            $audioRecordings = App\CandidateRecording::where('candidate_id', $candidateInfo->id)
                                                                ->where('assesment_id', App\Assesment::where('slug', $candidateInfo->assesment_slug)->first()->id)
                                                        ...
                                                        @if ($audioRecordings)
                                                            <br>
                                                            <span class="d-block mt-3" style="font-weight: 600">Recording
                                                                (During Exam)
                                                                : </span>
                                                            @php
                                                                $recordingsArray = [];
                                                            ...
                                                            <script>
                                                                ...
                                                                document.querySelector("#playlistSerial_" + <?php echo $index; ?>).innerHTML = "Playing 1/" + files<?php echo $index; ?>
                                                                    .length;
                                                                ...
                                                                function next<?php echo $index; ?>() {
                                                                    ...
                                                                    document.querySelector("#playlistSerial_" + <?php echo $index; ?>).innerHTML = "Playing " + Number(
                                                                        index<?php echo $index; ?> + 1) + "/" + files<?php echo $index; ?>.length;
                                                                }
                                                        ...
                                                        <span class="d-block mt-3" style="font-weight: 600">Webcam Capture
                                                            (During Exam) : </span>
                                                        <?php
                                                    ...
                                                    <td>
                                                        @if ($result->question_type == 1 || $result->question_type == 3)
                                                            @if ($result->question_created_by == Auth::user()->id)
                                                                ...
                                                                @endphp
                                                            @else
                                                                <i style="font-weight: 600;">Responses are kept confidential
                                                                    for ethical reasons and evaluations are performed by
                                                        ...
                                                        @else
                                                            <input style="width: 600px" class="form-control"
                                                                name="programming_language"
                                                                id="programming_language_{{ $sl }}"
                                                                value="{{ $result->pl_code }}" readonly>
                                                            <textarea style="width: 600px" class="code_answer" id="code_answer_{{ $sl }}" name="answer[]">{!! $result->answer !!}</textarea>
                                                            <a style="width: 600px"
                                                                class="btn btn-sm btn-success rounded mt-1 mb-2 text-white"
                                                                onclick="runMyCode({{ $sl }})">
                                                                <img src="{{ url('images') }}/loader3.gif"
                                                                    style="height:17px;width:17px;margin-right:5px;display:none"
                                                                    id="loading{{ $sl }}"><b>Execute</b>
                                                            </a>
                                                            <label style="width: 600px" class="d-block">See the Results Here
                                                                :</label>
                                                            <div id="result{{ $sl }}"
                                                                style="width: 600px; min-height: 100px; background:rgb(213, 213, 213); border-radius: 5px; padding: 5px 10px;font-weight:400 !important">
                                                    ...
                                                    <td style="text-align: center">
                                                        ...
                                                        @php
                                                            ...
                                                            $marksByEvaluator = App\CandidateResult::select('marks')
                                                                ->where([['evaluator_id', '=', null], ['candidate_id', '=', $result->candidate_id], ['assesment_id', '=', $result->assesment_id], ['question_id', '=', $result->question_id]])
                                                                ->first();

                                                            if (!$marksByEvaluator) {
                                                                $marksByEvaluator = App\CandidateResult::select('marks')
                                                                    ->where([['evaluator_id', '=', Auth::user()->id], ['candidate_id', '=', $result->candidate_id], ['assesment_id', '=', $result->assesment_id], ['question_id', '=', $result->question_id]])
                                                                    ->first();
                                                            }
                                                        ...
                                                        <input type="number" step=".01"
                                                            max="{{ App\QuestionBank::where('id', $result->question_id)->first()->marks }}"
                                                            @if ($result->question_type == 1 || $result->question_type == 3) value="{{ $result->marks }}" @else value="{{ $evaluatorMarks }}" @endif
                                                            name="marks[]" class="form-control p-1 text-center"
                                                            @if ($result->question_type == 1 || $result->question_type == 3) readonly @endif>
                                                        out of
                                                        <b>{{ App\QuestionBank::where('id', $result->question_id)->first()->marks }}</b>
                                                    </td>
                                                </tr>
                                                @php
                                                    $sl++;
