...
<div class="container" id="container">
    <div class="form-container sign-up-container">
        <form method="POST" action="{{ route('register') }}" onsubmit="validateForm()">
            ...
            <input id="email" type="email" onkeyup="checkBusinessEmail(this.value)" class="@error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" required
            ...
            <input id="password" type="password" class="@error('password') is-invalid @enderror register_password" name="password" required autocomplete="current-password" placeholder="Password" />
            @error('password')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
            ...
            <input id="password-confirm" class="register_confirm_password" type="password" placeholder="Confirm Password" name="password_confirmation" required autocomplete="new-password" required>

            <a href="javascript:void(0)" onclick="validateForm()" class="register_form_validaion" class="mt-2">Sign Up</a>
